...
// - Physical Device: Use 'http://13.203.232.71:4000/v1' (your computer's IP)
...
// - Physical Android Device: Use 'http://13.203.232.71:4000/v1' (your computer's IP)
// - iOS Simulator: Use 'http://localhost:3000/v1'
...
const DEFAULT_BACKEND_HOST = 'http://13.203.232.71';

// Helper: Determine the correct API URL based on platform and environment
// Using AWS server at 13.203.232.71 for all platforms
const getApiBaseUrl = () => {
  if (!__DEV__) {
...
export const videoApi = axios.create({
  baseURL: API_BASE_URL,
  timeout: 300000, // 5 minutes for video uploads (large files can take time)
  headers: {
    'Content-Type': 'application/json',
  },
  ...
  async (error) => {
    ...
    if (error.response) {
      ...
      if (error.code === 'ERR_NETWORK' || error.code === 'ECONNREFUSED') {
        ...
        const port = baseUrl.split(':').pop() || '4000';
        const serverIp = baseUrl.replace('http://', '').split(':')[0];
        const isAwsServer = serverIp === '13.203.232.71';
