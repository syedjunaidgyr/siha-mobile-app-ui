...
// - Physical Device: Use 'http://35.154.207.79:4000/v1' (your computer's IP)
...
// - Physical Android Device: Use 'http://35.154.207.79:4000/v1' (your computer's IP)
// - iOS Simulator: Use 'http://localhost:3000/v1'
...
const DEFAULT_BACKEND_HOST = 'http://35.154.207.79';

// Helper: Determine the correct API URL based on platform and environment
// Using AWS server at 35.154.207.79 for all platforms
const getApiBaseUrl = () => {
  if (!__DEV__) {
  ...
  async (error) => {
    ...
    if (error.response) {
      ...
      if (error.code === 'ERR_NETWORK' || error.code === 'ECONNREFUSED') {
        ...
        const port = baseUrl.split(':').pop() || '4000';
        const serverIp = baseUrl.replace('http://', '').split(':')[0];
        const isAwsServer = serverIp === '35.154.207.79';
